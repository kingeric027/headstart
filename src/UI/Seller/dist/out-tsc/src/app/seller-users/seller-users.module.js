import { __decorate } from "tslib";
import { NgModule } from '@angular/core';
import { SharedModule } from '@app-seller/shared';
import { SellerUserTableComponent } from './components/seller-user-table/seller-user-table.component';
import { SellerUsersRoutingModule } from './seller-users-routing.module';
var SellerUsersModule = /** @class */ (function () {
    function SellerUsersModule() {
    }
    SellerUsersModule = __decorate([
        NgModule({
            imports: [SharedModule, SellerUsersRoutingModule],
            declarations: [SellerUserTableComponent],
        })
    ], SellerUsersModule);
    return SellerUsersModule;
}());
export { SellerUsersModule };
//# sourceMappingURL=seller-users.module.js.map