...
describe('CacheInterceptor', function () {
    ...
    describe('non-get requests', function () {
        var nonGetHttpVerbs = ['post', 'put', 'patch', 'delete'];
        beforeEach(function () {
        ...
        nonGetHttpVerbs.forEach(function (verb) {
            it('should not be cached', function () {
                // create a request for each non-get verb and assert that no cache headers are added
                httpClient[verb]('some-mock-url').subscribe(function (response) {
                    expect(response).toBeTruthy();
                });
                var req = httpMock.expectOne('some-mock-url');
    ...
    it('should not cache non-ie requests', function () {
        ...
        httpClient.get('some-mock-url').subscribe(function (response) {
            expect(response).toBeTruthy();
        });
        var req = httpMock.expectOne('some-mock-url');
        expect(req.request.headers.get('Cache-Control')).toBeNull();
    ...
    it('IE11 get requests should be cached', function () {
        // mocking user agent to be IE11
        window.navigator['__defineGetter__']('userAgent', function () {
            return 'Mozilla/5.0 (Windows NT 10.0; WOW64; Trident/7.0; rv:11.0) like Gecko';
        });
        ...
        expect(req.request.headers.get('Expires')).toBe('Sat, 01 Jan 2000 00:00:00 GMT');
        req.flush({ hello: 'world' });
        httpMock.verify();
    });
});
