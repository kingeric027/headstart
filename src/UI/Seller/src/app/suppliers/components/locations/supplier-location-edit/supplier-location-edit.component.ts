...
import { ValidateCAZip, ValidateUSZip, ValidatePhone } from '@app-seller/validators/validators';
import { takeWhile } from 'rxjs/operators';
import { SupplierAddressService } from '../supplier-address.service';
import { OrderCloudIntegrationsConversionRate, HeadStartSDK } from '@ordercloud/headstart-sdk';
@Component({
...
export class SupplierLocationEditComponent implements OnChanges {
  ...
  zipPlaceholder: string = 'Enter Zip';
  stateLabel: string = 'State';
  statePlaceholder: string = 'Enter State';
  cityLabel: string = 'City';
  cityPlaceholder: string = 'Enter City';
  street1Label: string = 'Street 1';
  street1Placeholder: string = 'Enter Street 1';
  countryHasBeenSelected: boolean = false;
  ...
  createSupplierLocationForm(supplierLocation: Address) {
    this.resourceForm = new FormGroup({
      ...
      Zip: new FormControl({value: supplierLocation.Zip, disabled: true}, [Validators.required, ValidateUSZip || ValidateCAZip]),
  ...
  setZipValidator() {
    ...
    this.countryHasBeenSelected = this.resourceForm.controls['Country'].value !== "";
    this.resourceForm
      .get('Country')
  ...
  getCurrencyFromCode = (code: string) => this.countryOptions.find(c => c.abbreviation === code)?.currency;

  getFlagForCountry(currency: string): string {
    if (currency === "") return undefined;
    const flagMatch = this.currencyOptions?.find(c => c.Currency.toString() === currency)?.Icon;
