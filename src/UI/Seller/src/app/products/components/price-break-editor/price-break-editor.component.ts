...
import { PriceBreak, PriceSchedule, OcBuyerService } from '@ordercloud/angular-sdk';
...
export class PriceBreakEditor {
  ...
  getPriceBreakRange(index: number): string {
    ...
    let rangeString;
    if (priceBreaks[index].Quantity === priceBreaks[indexOfNextPriceBreak]?.Quantity - 1) {
      rangeString = priceBreaks[index].Quantity;
    } else if (indexOfNextPriceBreak < priceBreaks.length) {
      rangeString = `${priceBreaks[index].Quantity} - ${priceBreaks[indexOfNextPriceBreak].Quantity - 1}`;
    } else {
      rangeString = `${priceBreaks[index].Quantity}+`;
    }
    return rangeString;
  ...
  handlePriceBreakErrors(priceBreaks: PriceBreak[]): boolean {
    let hasError = false;
    if (priceBreaks.some(pb => pb.Price === Number(this.newPriceBreak.Price))) {
      this.toasterService.error('A Price Break with that price already exists');
      hasError = true;
    }
    if (priceBreaks.some(pb => pb.Quantity === Number(this.newPriceBreak.Quantity))) {
