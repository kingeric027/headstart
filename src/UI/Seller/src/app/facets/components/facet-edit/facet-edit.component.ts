...
export class FacetEditComponent implements OnInit {
    ...
    constructor(public facetService: FacetService, private toaster: ToastrService, private router: Router) {}
    ...
    refreshFacetData(facet: ProductFacet): void {
        ...
        this.areChanges = this.facetService.checkForChanges(this._facetEditable, this._facetStatic);
        this.createProductFacetForm(facet);
    }
    ...
    updateFacetResource(facetUpdate: any): void {
        const resourceToUpdate = this._facetEditable || this.facetService.emptyResource;
        this._facetEditable = this.facetService.getUpdatedEditableResource(facetUpdate, resourceToUpdate);
        this.areChanges = this.facetService.checkForChanges(this._facetEditable, this._facetStatic);
      }
    ...
    getSaveBtnText(): string {
        return this.facetService.getSaveBtnText(this.dataIsSaving, this.isCreatingNew)
    }
}
