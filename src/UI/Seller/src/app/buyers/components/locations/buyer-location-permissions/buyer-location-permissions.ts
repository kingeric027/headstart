...
export class BuyerLocationPermissions {
  ...
  constructor(private buyerLocationService: BuyerLocationService, private buyerUserService: BuyerUserService) {}
  ...
  async updateUserPermissionAssignments(locationID: string): Promise<void> {
    ...
    this.locationPermissionsAssigmentsStatic = JSON.parse(JSON.stringify(this.locationPermissionsAssigmentsEditable));
  ...
  async executeUserUserGroupAssignmentRequests(): Promise<void> {
    ...
    await this.buyerUserService.updateUserUserGroupAssignments(this._locationID.split('-')[0], this.add, this.del);
