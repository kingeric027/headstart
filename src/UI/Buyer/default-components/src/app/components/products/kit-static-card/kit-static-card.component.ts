...
export class OCMKitStaticCard {
    ...
    return {
      ...
      xp: {
        ImageUrl: this.specFormService.getLineItemImageUrl(
          this._productKitDetails.Images, this._productKitDetails.Specs, this.specForm
        ),
  ...
  filterStaticKitSpecs(_productKitDetails: MeProductInKit): Spec[] {
    ...
    const filteredSpecs = _productKitDetails.Specs
      .filter(spec => allowedSpecs.find(s => s.SpecID === spec.ID))
      .map(spec => {
  ...
  buildStaticKitSpecs(_productKitDetails: MeProductInKit): VariantSpec[] {
    ...
    const variant = _productKitDetails.Variants.find((v: any) => v.xp.SpecCombo === specCombo);
    if (!variant) {
      return []
    }
