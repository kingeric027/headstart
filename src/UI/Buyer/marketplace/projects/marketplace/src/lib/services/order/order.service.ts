...
export class CurrentOrderService implements ICurrentOrder {
  ...
  constructor(public cart: CartService, public checkout: CheckoutService, private state: OrderStateService, private ocLineItemService: OcLineItemService, private ocOrderService: OcOrderService) { }
  ...
  async submitQuoteOrder(orderDetails: Order, lineItem: LineItem): Promise<Order> {
    ...
    const submittedQuoteOrder = await this.ocOrderService.Submit('Outgoing', quoteOrder.ID).toPromise().then(order => { return order });
