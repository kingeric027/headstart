                ...
                switch (key.ToLower())
                {
                    ...
                    case "page":
                        if (int.TryParse(value, out i) && i >= 1)
                            ...
                            throw new ApiErrorException.UserErrorException("page must be an integer greater than or equal to 1.");
                        break;
                    case "pagesize":
                        if (int.TryParse(value, out i) && i >= 1 && i <= 100)
                            listArgs.PageSize = i;
                        else
                            throw new ApiErrorException.UserErrorException($"pageSize must be an integer between 1 and 100.");
                        break;
                    case "search":
